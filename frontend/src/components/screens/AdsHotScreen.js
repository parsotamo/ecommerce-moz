...
import { fetchHotProducts } from '../../actions';
...
const AdsPopularScreen = ({ history }) => {
  ...
  const { loading, hotProducts, error, success, page, pages } = useSelector(
    (state) => state.hotProducts
  );

  useEffect(() => {
    dispatch(fetchHotProducts(history.location.search));
    return () => {
      dispatch({ type: 'FETCH_HOT_PRODUCTS_RESET' });
    };
