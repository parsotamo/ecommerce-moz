...
const validationErrorHandler = error =>{
    const messages = Object.values(error.errors).map(el => el.message).join(". ");
...
const duplicateKeyErrorHandler = error =>{
    ...
    console.log(value);
    return new AppError(`Erro de duplicação de dados. Este dados já existem no sistéma ${value}.`, 400)
}

const jsonWebTokenErrorHandler = error =>{
    return new AppError(`Erro de token: ${error.message}. Faça login de novo`, 400);
}
